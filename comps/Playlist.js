...
import SpeakingCardsAct from "./acts/SpeakingCardsAct";
import JumbledWordsAct from "./acts/JumbledWordsAct";
...
export default function Playlist(props) {
  ...
  async function onSelect(item, activeChap, i = 0, isManual = false) {
    ...
    if (!isSameChapter || !bg)
      bg = getCategoryBackground(props.toc.list[activeChap]?.label, playlistId);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (items && items.length > 0) {
        onSelect(items[0], state.activeChap, 0);
        return;
      }
    }
  ...
  const activeLabel =
    state.active?.label ||
    (state.activeChap >= 0 ? props.toc.list[state.activeChap]?.label : "");
  const userName =
    (typeof window !== "undefined" && localStorage.getItem("user_name")) || "U";
          ...
          <div className="sidebar">
            ...
            <div className="topicHeader">
              ...
              <div className="topicSub">
                {props.toc.label || "Learning Activities"}
              </div>
            </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setState({
                            ...
                            activeChap: i,
                            toggleChaps: tc,
                            currentBg: getCategoryBackground(
                              props.toc.list[i]?.label,
                              playlistId,
                            ),
                          });
                          ...
                          <div
                            ...
                            onClick={() =>
                              Array.isArray(item.data)
                                ? numberSelect(item, i, 0, {
                                    stopPropagation: () => {},
                                  })
                                : onSelect(item, i, j, true)
                            }
                          >
                            <div
                              className={`actNum${isActive ? " activeNum" : ""}`}
                            >
                              {j + 1}
                            </div>
                            <div
                              className={`actName${isActive ? " activeName" : ""}`}
                            >
                              {item.label}
                            </div>
                          </div>
          ...
          {!state.hideTOC && (
            <div className="topbar">
              ...
              <div className="topbarRight">
                {state.activeNum > 0 &&
                  props.toc.list[state.activeChap]?.list?.length > 0 && (
                    <div className="scorePill">
                      Activity{" "}
                      <span className="scoreNum">{state.activeNum}</span> of{" "}
          ...
          <div className="actContent">
            {state.active && state.active.type === "chapter" && (
              <div className="chapDisplay">
                <div style={{ textDecoration: "underline", color: "#7a9872" }}>
            ...
            {state.isLoading && (
              <div className="loadingWrap">
                <img
                  ...
                  src="/konzeptes/parrot-loader.jpg"
                  alt="loading"
                />
              </div>
            )}
...
function displayResource(
  ...
  if (item.loadError) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: "1.1rem", fontWeight: 600, color: "#3d5538" }}>
  ...
  switch (item.type) {
    ...
    case "speakingCards":
      return (
        <ActivityWrapper bgUrl={bgUrl} id={chapId}>
          <SpeakingCardsAct data={payload} />
        </ActivityWrapper>
      );
      case "jumbledWords":
      return (
        <ActivityWrapper bgUrl={bgUrl} id={chapId}>
          <JumbledWordsAct data={payload} />
        </ActivityWrapper>
      );
    case "completeWord":
      return (
    ...
    case "visualInformationProcessing":
    case "visual_audio":
    case "visual_image":
      return (
        <ActivityWrapper bgUrl={bgUrl} id={chapId}>
